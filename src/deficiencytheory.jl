...
function isregular(rn::ReactionSystem)
    ...
    isconsistent(rn) && (length(lcs) == length(tlcs)) &&
...
function solveconstraints(rn::ReactionSystem, model::Model, confluence::Vector,
        partition::Array, cutdict::Dict; M = 1E8, ϵ = 1E-3)
    Y = complexstoichmat(rn);
    S = netstoichmat(rn)
    μ = model[:μ];
    n = model[:n]
    ...
    @constraint(model, UM[u = partition[1], m = partition[2]],
        Y[:, u]' * μ - Y[:, m]' * μ ≥ ϵ)
    @constraint(model, UL[u = partition[1], l = partition[3]],
        Y[:, u]' * μ - Y[:, l]' * μ ≥ ϵ)
    @constraint(model, ML[m = partition[2], l = partition[3]],
        Y[:, m]' * μ - Y[:, l]' * μ ≥ ϵ)
