...
Apply the deficiency-one multistationarity criterion.

# Arguments

- `rn`: a regular reaction system with deficiency one.

# Returns

`true` when the criterion finds a feasible multistationarity witness and
`false` otherwise.

# Throws

An error when `rn` is not a regular deficiency-one reaction system.
"""
function deficiencyonealgorithm(rn::ReactionSystem)
    (Catalyst.deficiency(rn) == 1 && isregular(rn)) ||
        error("The deficiency one algorithm only works for regular deficiency one networks.")
...
function isregular(rn::ReactionSystem)
    ...
    return isconsistent(rn) && (length(lcs) == length(tlcs)) &&
        all(Graphs.is_tree ∘ SimpleGraph, tlc_graphs)
end
...
function solveconstraints(
        rn::ReactionSystem, model::Model, confluence::Vector,
        partition::Array, cutdict::Dict; M = 1.0e8, ϵ = 1.0e-3
    )
    Y = complexstoichmat(rn)
    S = netstoichmat(rn)
    μ = model[:μ]
    ...
    @constraint(
        model, UM[u = partition[1], m = partition[2]],
        Y[:, u]' * μ - Y[:, m]' * μ ≥ ϵ
    )
    @constraint(
        model, UL[u = partition[1], l = partition[3]],
        Y[:, u]' * μ - Y[:, l]' * μ ≥ ϵ
    )
    @constraint(
        model, ML[m = partition[2], l = partition[3]],
        Y[:, m]' * μ - Y[:, l]' * μ ≥ ϵ
    )
    ...
    for edge in keys(cutdict)
        ...
        elseif cutsum > 0
            sr ∈ partition[1] ?
                @constraint(
                    model, (Y[:, sr]' * μ - Y[:, ds]' * μ) ≥ ϵ,
                    base_name = "Edge $sr-$ds"
                ) :
                @constraint(
                    model, (Y[:, sr]' * μ - Y[:, ds]' * μ) ≤ -ϵ,
                    base_name = "Edge $sr-$ds"
...
function generatepartitions(rn::ReactionSystem)
    ...
    for i in 0:(numparts - 1)
        ...
        num_trivial_tlcs == 0 && (isempty(partition[1]) || isempty(partition[3])) &&
            continue

        # If the number of trivial terminal linkage classes is one, we can ignore
        # the partition for which both U and L are empty
        num_trivial_tlcs == 1 && (isempty(partition[1]) && isempty(partition[3])) &&
            continue
