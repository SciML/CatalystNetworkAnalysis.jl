...
Existence for steady states with certain properties can be checked from the network structure: 
...
This functionality is in the function `hasuniqueequilibria(rn)`. The uniqueness of steady states checks proceeds as follows. The function terminates every time a network is conclusively determined to have unique steady states or multi-stationarity.  
...
Future desiderata for this check include methods from **monotone systems theory**,**degree theory**, and **embedded multi-stationary networks** 

Other functionality related to this uniqueness: 
```@docs
